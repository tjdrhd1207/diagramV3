...
import { $DummyFlowXML, createDummyFlowXML } from "@/consts/server-object";

interface CreateProjectInfos {
    projectID: string;
...
export const createProject = async (props: CreateProjectInfos) => {
    const prefix = "createProject";
    const { projectID, workspaceName, projectName, projectDescription } = props;
    assert(projectID, "projectID is empty");
    assert(workspaceName, "workspaceName is empty");
    assert(projectName, "projectName is empty");
    assert(projectDescription, "projectDescription is empty");
    ...
    try {
        ...
        let sqlResult = await transaction.request()
            ...
            .query(`INSERT INTO PROJECT_INFORMATION (
                PROJECT_ID, WORKSPACE_NAME, PROJECT_NAME, PROJECT_DESCRIPTION, CREATE_DATE, CREATE_TIME, UPDATE_DATE, UPDATE_TIME
            ) VALUES (
                @projectID, @workspaceName, @projectName, @projectDescription, @createDate, @createTime, @createDate, @createTime
            )`);
        ...
        const ivrmainSource = createDummyFlowXML("ivrmain");
...
export const getProjectInfoList = async () => {
...
export const createFlow = async (projectID: string, flowName: string, flowTag: string ) => {
    ...
    try {
        ...
        logger.debug("DB transaction started", { prefix: prefix });
        const { yyyymmdd: createDate, hhmmss: createTime } = getNowDateTime();
        const flowSource = createDummyFlowXML(flowName);
        logger.debug(`projectID: ${projectID}`, { prefix: prefix });
